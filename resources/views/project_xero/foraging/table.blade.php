<div>
    ...
    <div class="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-8 w-full mb-8">
        ...
        <div class="bg-white h-28 p-4 rounded-md shadow-md flex flex-col items-center justify-around">
            <h4>Latest Update</h4>
            <b class="text-2xl">{{ $updated ? $updated->diffForHumans() : ''}}</b>
            <span />
        </div>
        <div class="bg-white h-28 p-4 rounded-md shadow-md flex flex-col items-center justify-around">
            <h4>Highest Average Value</h4>
            <b class="text-2xl">{{ $bestVal }}</b>
            <span />
        </div>
    </div>
    <div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8 w-full">
        @foreach($locations as $location)
            ...
            @if(($massedit) || (in_array($location->type, ['Standard', 'Monthly']) || ($location->type == 'Event' && $eventShow == true)))
                <div class="col-span-1 my-6 rounded-md shadow-md px-2 py-4" style="background-color: {{ $location->color }}">
